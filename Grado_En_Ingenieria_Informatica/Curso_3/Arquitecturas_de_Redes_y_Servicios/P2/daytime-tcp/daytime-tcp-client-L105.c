...
#define ERROR_ESTABLECIMIENTO_CONEXION "Se ha producido un error en el establecimiento de la conexion"
#define MAX_TAM 1000
...
void cerrarConexion(int socketfd,int nbytes,char *respuesta);
...
int main(int argc, char **argv){
    ...
    //Definicion de una variable para guardar el numero de bytes leidos del buffer tras el cierre de la conexion
    int nbytes=0;
	...
	if(argc < 2 || argc > 4 || argc == 3){
		fprintf(stderr,ERROR_NUMERO_ARGUMENTOS);
		exit(-1);
	}
	if(argc == 2){
		...
		if(ipValida(argv[1]) == false){
			fprintf(stderr,ERROR_IP_ERRONEA);
			exit(-1);
		}
		
		//Obtenemos el numero de puerto bien conocico al que esta asignado por defecto el servico daytime
		if((servicio = getservbyname("daytime","udp")) == NULL){
			fprintf(stderr,ERROR_BUSQUEDA_SERVICIO);
			exit(-1);
		}
		
		//Una vez que hemos obtenido el numero de puerto se lo asignamos a la variable puerto 
		puerto = ntohs(servicio->s_port);

	}
	else{
		
		// Si el numero de argumentos introducidos por el usuario no son 2, comprobamos que el formato de la IP sea valido y en caso contrario, mostramos un error
		if(ipValida(argv[1]) == false){
			fprintf(stderr,ERROR_IP_ERRONEA);
			exit(-1);
		}
		...
		if(opcionCorrecta(argv[2]) == false){
			fprintf(stderr,ERROR_OPCION_INCORRECTA);
			exit(-1);
		}
		
		//Comprobacion de que el puerto que introduzca el usuario solo tenga digitos, en caso de que esto no sea asi, mostramos un error
		if(soloDigitos(argv[3]) == false){
			fprintf(stderr,ERROR_FORMATO_PUERTO);
			exit(-1);
		}
		...
		if(puertoValido(atoi(argv[3])) == false){
			fprintf(stderr,ERROR_PUERTO_ERRONEO);
			exit(-1);
		}
	...
	//Creamos el socket con la funcion socket, en caso de que de un error mostramos un mensaje
	if((socketfd = socket(AF_INET,SOCK_STREAM,0)) == -1){
		fprintf(stderr,ERROR_CREACION_SOCKET);
		exit(-1);
	}

	//Especificamos que se van a usar direcciones IPv4
	...
	//Inicializamos el puerto del servidor al que vamos a mandar los mensajes, con la funcion htons convertimos el puerto de tipo int a formato de red
	...
	if((inet_aton(direccionDestino,(struct in_addr *) &(direccionServidor.sin_addr.s_addr))) == 0){
		...
		exit(-1);
		cerrarConexion(socketfd,nbytes,respuesta);		
	}

	//Establecemos una conexion con el servidor y en caso de que de error, mostramos un mensaje
    	if((connect(socketfd,(struct sockaddr *) &(direccionServidor),sizeof(direccionServidor))) == -1){
        	fprintf(stderr,ERROR_ESTABLECIMIENTO_CONEXION);
        	exit(-1);
		cerrarConexion(socketfd,nbytes,respuesta);
    	}
	
	
	//Mandamos el mensaje al servidor y en caso de que este no se mande correctamente y de un error, mostramos un mensaje
	if((send(socketfd,(void *) pregunta,sizeof(pregunta),0)) == -1){
		fprintf(stderr,ERROR_ENVIO_DATOS);
		exit(-1);
		//Si se ha producido un error, llamamos a la funcion cerrarConexion para cerrae el socket
		cerrarConexion(socketfd,nbytes,respuesta);
	}

	//Recibimos el mensaje de respuesta del servidor y lo guardamos en la variable respuesta, en caso de que de error, mostramos un mensaje
	if((recv(socketfd,respuesta,sizeof(respuesta),0)) == -1){
		fprintf(stderr,ERROR_RECIBIMIENTO_DATOS);
		exit(-1);
		//Si se ha producido un error, llamamos a la funcion cerrarConexion para cerrar el socket
		cerrarConexion(socketfd,nbytes,respuesta);
	}
...
void cerrarConexion(int socketfd,int nbytes, char *respuesta){
	...
	//Cerramos la conexion con el servidor cerrando la recepcion y envio de mensajes
    shutdown(socketfd,SHUT_RDWR);
    
	//Leemos el numero de byte restantes en el buffer de lectura y en caso de que de error, mostramos un mensaje
    if((nbytes=recv(socketfd,respuesta,sizeof(respuesta),0)) == -1){
		fprintf(stderr,ERROR_RECIBIMIENTO_DATOS);
		close(socketfd);
		exit(-1);
	}
    
	//Si no hay bytes restantes en el buffer de lectura, cerramos el socket
    else if(nbytes == 0){
	    close(socketfd);
    }
}
